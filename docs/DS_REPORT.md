...
| модель | MAPE_common | skill vs struct | DM p |
|---|---|---|---|
| **structural_osl** | **13.7%** | 0 (бейзлайн) | — |
| hist_gbm | 12.1% | +0.12 | 0.66 |
| elasticnet | 40.8% | −1.97 | 0.024 |
| ridge | 41.8% | −2.05 | 0.020 |

**Вывод (честный, без overclaim):** на N=24 **ни одна модель статистически значимо НЕ
бьёт структурный prior**. GBM номинально лучше на 1.6 п.п., но **Diebold–Mariano p=0.66**
— различие в пределах шума. Регуляризованная линейная переобучается на экстраполяции цен
(скачок золота 2025). **Зрелый DS-вывод: на таком объёме данных доменный prior — правильный
выбор, а ML-гибкость не оправдана; мы это доказали out-of-sample, а не декларировали.**
...
**Результат:** OOS-покрытие **6/6 = 100%** (цель 90%, консервативно из-за n_calib=5),
q=0.247. Маржинальная гарантия ≥1−α подтверждена на синтетике (обмениваемые остатки).
...
- **202 pytest зелёных, 0 skipped** (136 + 1 skip → 196 DS-доработкой → 202 с продуктовым слоем). CI (GitHub Actions), ruff/black.
...
- Тесты: leakage-гарды (train<test, scaler/FE/k_ только на train), метрики, DM, conformal-покрытие.
