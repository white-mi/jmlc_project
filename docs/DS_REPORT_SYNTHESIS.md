---
tags: [russian-propagation, ds, синтез, кросс-отрасль]
дата: "2026-07-23"
версия: "0.9"
---

# DS-синтез — кросс-отраслевой разбор (5 отраслей OSL)

> Сводит воедино пять DS-отчётов — [металлургия](DS_REPORT.md) · [нефтегаз](DS_REPORT_OILGAS.md) ·
> [химия](DS_REPORT_CHEMISTRY.md) · [энергетика](DS_REPORT_ENERGY.md) · [ОИВ](DS_REPORT_OIV.md).
> Каждый — реальная панель (эмитент×год FY2021–2025; ОИВ — регион×год FY2017–2024), честная
> out-of-sample walk-forward + split-conformal, наивные бейзлайны, явные ограничения. Здесь —
> **один нарратив поверх пяти**.
...
> транскрибированы из пяти DS-отчётов (в JSON их нет). Графики и сводная таблица
...
На пяти отраслях (N=18–30) ни в одной learned-модель не бьёт разумный бейзлайн со
...
| ОИВ | 24 | 4 региона (Башкортостан, Пермский край, Сахалинская обл., Татарстан) | **отложена** | нет ХМАО/ЯНАО (геоблок источников); scope-гетерогенность; добыча — почти сплошной gap |

«Плохие клиенты», на которых модель не ложится, **исключены** (Уралхим/НКНХ в химии,
Интер РАО/Росатом в энергетике, Томск/Оренбург в ОИВ) — не подгонка выборки, а отказ от эмитентов
без Q×P-структуры/отчётности (в ОИВ — от рядов, ломающих внутреннюю однородность scope).

---

## 3. Walk-forward — кросс-отраслево

Expanding-window, фолды 2022→2025 (ОИВ — 2018→2024). MAPE_common — на общем наборе *n*, где **все** модели дали
...
| ОИВ | 24 (22) | — отложена | 15.1% (ridge) | 16.9% | **ridge** | 15.1% | 0.347 (н.з.) |

*N — размер панели; n в скобках — общий набор для MAPE_common. DM p — победителя против **базы**
сравнения (структурная, где активна; в нефтегазе и ОИВ структурная отложена → база = persistence,
поэтому 0.053 — это hist_gbm против persistence, а 0.347 — ridge против persistence). Полная
таблица (+conformal) —
[`figures/synthesis/summary_table.md`](figures/synthesis/summary_table.md), авто-сборка из JSON.*

**Все DM p>0.05** ⇒ ни один победитель не отделён от базы статистически. Это не
слабость метода, а свойство данных: 4–6 эмитентов × 5 лет (ОИВ — 4 региона за 8 лет, 24 строки) —
слишком мало, чтобы различать модели. Любое «X% лучше Y%» здесь — **направление, а не доказательство**.

---

## 4. Пять режимов данных
...
| Панель регион×год, цены без объёмов, непрозрачная налоговая механика | ОИВ | learned (ridge) | нефтяной фискальный сигнал (`urals`×FX) + within-issuer FE дают skill **+10,5%** к наивному; гибкий GBM на 22 точках **переобучается** |
...
| ОИВ | ridge | 1/2 (50%) | ±28.1% | структурная отложена; test n=2 → неинформативно (в 2024 бюджеты выросли быстрее нефтяного сигнала) |

> [!warning] Честность conformal
> calib- и test-наборы малы (n=2–6) → покрытие **шумное**, не «out-of-sample 96%»; в ОИВ хвост
> вырождается до двух объектов, и 1/2 не несёт статистического содержания. Энергетика (N=30) —
...
- **ОИВ** — структурная фискальная Q×P отложена (расщепление налога на прибыль, КГН, НДФЛ,
  трансферты непрозрачны для честной формулы) → база = persistence; флагманские ХМАО/ЯНАО
  отсутствуют (финансовые порталы, форма 0503317 Казначейства и ЕМИСС геоблокированы); scope
  раскрытия разный по регионам (консолидированный / краевой) — снимается within-issuer FE, но
  остаётся ограничением; погодовая региональная добыча почти везде `null`.
...
подобранной метрикой (структурная энергетики — честные 11.88% на чистой физике Q×P, без внефизических
...
**Потолок малого N — сквозное ограничение всех пяти панелей.** Смена парадигмы на панель
регион×год (ОИВ) снимает его лишь в принципе: масштаб такой панели растёт с числом субъектов, но
фактически собрано **24 строки — 4 региона за 8 лет**, потому что источники по флагманским ХМАО/ЯНАО
(финансовые порталы, форма 0503317 Казначейства, ЕМИСС) недостижимы инструментами. Поэтому
DM-значимость не достигнута и здесь (ridge против persistence — `p=0.347`), и это зафиксировано
как есть, а не обойдено. Дальнейшие направления: накопление лет истории, расширение регионального
охвата при появлении доступа к источникам и калибровка на исторических РФ-шоках (COVID-2020,
санкции-2022, цикл ставки) для проверки реакции слоёв L1→L3.
