...
<div class="stat"><b>349 тестов · CI</b><span>0 skipped</span></div>
<div class="stat"><b>5 отраслей</b><span>walk-forward + conformal</span></div>
</div>

<!--
(20 сек): Russian Propagation превращает новость в оценку изменения вероятности дефолта по
...
| Модель (металлургия, N=24 (n=16)) | MAPE | DM p |
|---|--:|--:|
| persistence (наив) | **11.0%** | 0.41 |
| issuer_mean (наив) | 11.1% | 0.43 |
| hist_gbm | 12.1% | 0.66 |
...
**Значимый методологический вывод:** на N=24 **ничто не различимо** — даже наивный persistence
(11.0%) номинально лучше structural (13.7%), но все DM $p>0.4$ (внутри шума); линейные
переобучаются ($p<0.03$).
...
тест Диболда-Мариано против наивной базы. На металлургии наивный persistence (11.0%) номинально
лучший, но все p>0.4 — это шум; линейные переобучаются. Значимый вывод: на малых панелях доменный приор не
...
**Инженерия**
- **30 модулей** (+6 в agents/rag) · **≈11k строк** · **349 тестов**, 0 skipped
...
- Haiku 4.5 → **94% / 96%** (main 47/50 · sub 48/50); Sonnet 5 → **14/15** (подмножество N=15)
- **Реальный набор N=93** настоящих РУ-новостей с провенансом (silver-метки, аннотатор Opus ≠ Haiku): main **97%**, sub **86%**, non-boundary **90%** — честнее синтетики (реальный текст труднее), по main даже сильнее; разбор + confusion-матрица в `docs/L0_EVAL.md`
...
<!--
(35 сек): Инженерно — 30 модулей, 11 тысяч строк, 349 тестов, CI из шести джобов с Docker
...
Контракт промптов L0 закреплён офлайн-стражем: каждый ключ подстановки orchestrator обязан
присутствовать как `<KEY>` в промпте агента (иначе текст не инжектится). Тест:
...
| Тир | Отрасли |
|---|---|
| **Валидированы** (5) | металлургия, нефтегаз, химия, энергетика, ОИВ |
| **Иллюстративны** (2) | фарма, розница |
...
**Тесты** 349 (0 skipped): anti-leakage guards, bifurcation, conformal-split, JSON-контракт
