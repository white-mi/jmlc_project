...
@pytest.mark.parametrize('industry', ['metallurgy', 'oilgas', 'chemistry',
...
def test_spillover_oilgas_to_oiv_significant():
    ...
    assert r.impacts['oiv'] >= 0.5, (
        f'oilgas → oiv too weak: {r.impacts["oiv"]}'
    )
...
def test_segment_table_full_coverage():
    ...
    assert result['segments'] >= 10, (
        f'Expected ≥10 сегментов (10 base + опциональные подсегменты), '
        f'got {result["segments"]}'
    )
    assert result['subcategories'] >= 27, (
    ...
    for sgmt in normal:
        if abs(normal[sgmt].delta_pd) > 0.01:  # значимая база
            ...
            assert 2.0 < ratio < 2.5, (
                f'{sgmt}: ratio acute/normal = {ratio:.2f}, expected ~2.2'
            )
...
def test_kc_rate_increase_consumer_segments_worse():
    ...
    consumer_dominated = ('fl_massovy', 'fl_sredniy', 'fl_premium',
                           'sme_micro', 'sme_small', 'sme_mid', 'ml_mid_corp')
    for sgmt in consumer_dominated:
    ...
    for sub in ('1.1', '1.2', '4.1', '4.2'):
        ...
        for sgmt, imp in impacts.items():
            assert imp.delta_churn >= 0, (
                f'shock={sub} sgmt={sgmt}: Δchurn должен быть ≥0, '
                f'got {imp.delta_churn}'
            )
...
def test_legacy_direction_inverts_signs():
    ...
    for sgmt in classical:
        if abs(classical[sgmt].delta_pd) > 0.01:
            assert (classical[sgmt].delta_pd > 0) != (inverted[sgmt].delta_pd > 0), (
...
def test_pipeline_smoke_runs_to_completion():
    ...
    state = run_full_pipeline(
        smoke_shock='1.1', smoke_industry='oilgas',
        date='2026-04-25', skip_l0=False,
    )
    ...
    assert state['L1_macro']['phase'] in ('expansion', 'late-cycle',
                                           'recovery', 'contraction')
    # OSL должен возвращать данные хоть для одной отрасли
    osl = state['L1_5_osl']
    assert any('emitents' in data for data in osl.values()), (
        'No OSL data returned for any industry'
    )
