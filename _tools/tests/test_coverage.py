...
def test_illustrative_industries_are_not_wired_as_structural():
    ...
    for ind, meta in _man()["industries"].items():
        ...
        assert (
            ind not in osl_models.INDUSTRY_PRICE_FEATURES
        ), f"{ind}: иллюстративный, но в PRICE_FEATURES"
        assert (
            ind not in ds_synthesis.INDUSTRIES
        ), f"{ind}: иллюстративный, но в ds_synthesis.INDUSTRIES"
...
def test_rejected_candidates_documented_and_absent():
    ...
    for k, v in rej.items():
        ...
        assert (
            k not in osl_models.INDUSTRY_PRICE_FEATURES
        ), f"{k} отклонён, но в INDUSTRY_PRICE_FEATURES"
...
def test_committed_doc_matches_generator():
    ...
    assert (
        gen == committed
    ), "COVERAGE_TIERS.md разошёлся с генератором — перегенери: python coverage.py"
