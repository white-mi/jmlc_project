...
ПОДКЛЮЧЕНА и ЧИСТАЯ двухкомпонентная: генерация×РСВ + мощность×КОМ, БЕЗ
...
def test_energy_pure_structural_not_inflated():
    ...
    Уход к ~8% означал бы, что в модель попали подобранные интерцепты по теплогенерации —
    такая точность берётся из фиттинга, а не из физики."""
    rows = _en()
    _need(rows)
    ...
    ), f"структурная MAPE {mape} подозрительно низка — проверь интерцепты на тепло"
