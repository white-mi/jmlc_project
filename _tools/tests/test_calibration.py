...
def test_apply_all_calibrations_returns_summary():
    ...
    expected = {'osl_metallurgy', 'osl_oilgas', 'osl_chemistry',
                'osl_pharma', 'osl_retail', 'osl_energy', 'osl_oiv'}
    assert set(summary.keys()) == expected


@pytest.mark.parametrize('module', [
    'osl_metallurgy', 'osl_oilgas', 'osl_chemistry',
    'osl_pharma', 'osl_retail', 'osl_energy', 'osl_oiv',
])
...
def test_tune_multi_period():
    ...
    best, metrics = tune_multi_period(predict, targets, period_shares,
                                       param_min=0, param_max=20)
