...
def test_high_confidence_rows_have_target():
    ...
    rows = _rows()
    _need(rows)
    bad = [(r.issuer, r.period) for r in rows
           if r.confidence == 'high' and not r.has_target]
...
def test_fy_period_months_is_12():
    ...
    for r in rows:
        if r.period_kind == 'FY':
            assert r.period_months == 12, f'{r.issuer} {r.period}: FY, но period_months={r.period_months}'
...
def test_price_window_within_reporting_period():
    ...
    for p in prices:
        ...
        if p.window_start < ps or p.window_end > pe:
            violations.append((p.period, p.series,
                               f'{p.window_start}..{p.window_end} вне {ps}..{pe}'))
...
def test_prices_joined_to_rows():
    ...
    rows = _rows()
    _need(rows)
    polyus = [r for r in rows if r.issuer == 'Полюс' and r.period in
              {p.period for p in osl_panel.load_prices() if p.series == 'lbma_gold'}]
    if not polyus:
...
def test_csv_headers_match_schema():
    ...
    assert rev_header == list(SCHEMA['revenue_columns']), 'panel_revenue.csv ≠ schema.revenue_columns'
...
def test_issuer_relevant_price_present():
    ...
    driver = {'Полюс': 'gold', 'Норникель': 'palladium',
              'Северсталь': 'steel_fob_chm', 'ММК': 'steel_fob_chm', 'НЛМК': 'steel_fob_chm'}
    blind = []
    for r in rows:
