...
def propagate_shock(
    source_industry: str, magnitude_pp: float = 1.0, matrix_path: Path = DATA_PATH
) -> SpilloverResult:
    ...
    if source_industry not in matrix:
        raise ValueError(f"Неизвестная отрасль: {source_industry}. " f"Допустимые: {industries}")
...
def main():
    ...
    parser.add_argument("--check", action="store_true", help="Только проверить инварианты матрицы")
