...
from spillover import propagate_shock, propagate_credit_channel, severity_to_magnitude
...
def _load_shock_map() -> dict:
    try:
        ...
        print(f"  ⚠️ Не удалось загрузить shock_to_industries.json: {e}", file=sys.stderr)
        return {}
...
def _load_brent_scenarios() -> dict:
    try:
        ...
        print(f"  ⚠️ Не удалось загрузить brent_scenarios.json: {e}", file=sys.stderr)
        return {}
...
def get_industries_for_shock(
    ...
    print(f"  ⚠️ routing fallback для подкатегории {subcategory!r}", file=sys.stderr)
...
def osl_for_industry(industry: str) -> dict:
    ...
    try:
        if industry == "metallurgy":
            ...
            results = [
                make_interval_generic(c, "osl_oilgas")
                for c in companies
                if c in m.ACTUAL_REVENUE_12M_2025
            ]
            ...
            results = [
                make_interval_generic(c, "osl_chemistry")
                for c in companies
                if c in m.ACTUAL_REVENUE_2025
            ]
        elif industry == "retail":
            results = [make_interval_retail(c) for c in ["Wildberries", "Ozon", "М.Видео"]]
        elif industry == "energy":
            results = [
                make_interval_energy(c)
                for c in ["Интер РАО", "РусГидро", "Юнипро", "Т Плюс", "Росатом-Энергоатом"]
            ]
        elif industry == "oiv":
            results = [
                make_interval_oiv(r)
                for r in ["ХМАО-Югра", "Тюменская обл.", "ЯНАО", "Татарстан", "Сахалинская обл."]
            ]
...
def run_full_pipeline(
    ...
    if smoke_shock:
        ...
        try:
            ...
            agent_state, _md = run_agents(
                news_text or "", source, state["date_news"], llm_mode=llm_mode
            )
    ...
    state["L1_5_osl"] = {ind: osl_for_industry(ind) for ind in industries}
...
def render_markdown(state: dict) -> str:
    ...
    lines.append(
        f'- **EPU (30d):** {l1.get("epu", "?")}/100 — общая частота uncertainty-текстов{epu_degraded_note}'
    )
    lines.append(
        f'  - **Negative-EPU:** {l1.get("epu_negative_pct", "?")}% '
    ...
    if "." in sub_used:
        lines.append(
            f"- **Подкатегория шока:** {sub_used} → L3 использует per-channel direction из таблицы (legacy direction игнорируется)"
        )
    else:
    ...
    for industry, data in osl.items():
        ...
        for em in data.get("emitents", []):
            inside = (
                "✅"
                if em.get("inside_interval")
                else ("❌" if em.get("inside_interval") is False else "—")
            )
    ...
    if fwd and fwd.get("companies"):
        ...
        lines.append(
            f'**Brent:** статус-кво ${fwd["brent_pre"]:.0f}/барр → '
        ...
        for company, d in fwd["companies"].items():
            delta_marker = "🔴" if d["delta_pct"] < -3 else ("🟢" if d["delta_pct"] > 3 else "⚪")
            lines.append(
    ...
    lines.append(
        f'**Источник шока:** {l2.get("source", "?")} (magnitude {l2.get("magnitude_pp", "?")} п.п.)'
    )
    lines.append("\n| Отрасль | ΔPD (п.п.) |")
    lines.append("|---|---|")
...
def main():
    ...
    state = run_full_pipeline(
        ...
        source=args.source,
        date=args.date,
        smoke_shock=smoke_shock,
        smoke_industry=smoke_industry,
