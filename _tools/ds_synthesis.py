...
CONFORMAL = {
    "metallurgy": {
        "model": "structural_osl",
        "coverage": "6/6",
        "pct": 100,
        "width": "±24.7%",
        "n_calib": 5,
        "note": "малый calib → артефакт малого N",
        "src": "DS_REPORT.md",
    },
    "oilgas": {
        "model": "hist_gbm",
        ...
        "width": "±28%",
        "n_calib": 4,
        "note": "структурная отложена; persistence 7/8=88%",
    ...
    "chemistry": {
        ...
        "width": "±20%",
        "n_calib": 4,
        "note": "структурный conformal ненадёжен (1/6=17%, объёмы разрежены)",
...
def _winner(summary: dict):
    ...
    cand = [
        (m, v.get("mape_common")) for m, v in summary.items() if v.get("mape_common") is not None
    ]
    return min(cand, key=lambda t: t[1]) if cand else (None, None)
...
def plot_mape(rows: list, out: Path) -> Path:
    ...
    w = 0.26
    series = [
        ("structural", "Структурная", "tab:blue"),
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([f'{lab}\n(N={r["panel_n"]})' for lab, r in zip(labels, rows)])
...
def plot_conformal(out: Path) -> Path:
    ...
    ax.set_xticks(xs)
    ax.set_xticklabels([lbl for _k, lbl, _c in items], fontsize=8)
    ax.set_ylim(0, 112)
    ...
    ax.annotate(
        "контемпоральные физданные года t\nотслеживают растущий тренд",
        xy=(0, 100),
        xytext=(0.3, 60),
        fontsize=8,
...
def write_table(rows: list, out: Path) -> Path:
    ...
    lines += [
        "",
        "**Conformal OOS-покрытие** (транскрипция из DS_REPORT_*.md; calib-набор мал n=2–6 "
        "→ шумно):",
        "",
    ...
    for ind in INDUSTRIES:
        c = CONFORMAL[ind]
        lines.append(
            f'| {RU[ind]} | {c["model"]} | {c["coverage"]} ({c["pct"]}%) | {c["width"]} | '
            f'{c["note"]} | {c["src"]} |'
        )
    lines.append("")
...
def run(out_dir: Path = None, metrics_dir: Path = METRICS_DIR) -> dict:
    ...
    if not metrics:
        print(
            f"Нет *_metrics.json в {metrics_dir}. Сначала: python osl_walkforward.py --industry <X>."
        )
        return {}
...
def main():
    ...
    for r in art["rows"]:
        print(
            f'  {RU[r["industry"]]:<12} N={r["panel_n"]:<3} победитель={r["winner"]:<14} '
