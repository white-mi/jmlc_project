...
PRICES_12M_2025: Dict[str, CommodityPrice] = {
    ...
    'steel_domestic_rub': CommodityPrice('steel_domestic_rub', 't', 65_000, '12M2025',
                                          'Минпромторг/Металлоторг — оценка'),
}
...
NORNICKEL_PRODUCTION = [
    ...
    ProductionData('Норникель', 'gold', 200_000, 'oz', '12M2025',
                   'байпродукт никелевой переработки'),
    ProductionData('Норникель', 'cobalt', 5_000, 't', '12M2025',
                   'байпродукт'),
    ProductionData('Норникель', 'rhodium', 80_000, 'oz', '12M2025',
                   'высокомаржинальный PGM'),
...
SEVERSTAL_PRODUCTION = [
    ProductionData('Северсталь', 'steel', 11_000_000, 't', '12M2025',
                   'consolidated steel output'),
]
...
NLMK_PRODUCTION = [
    ProductionData('НЛМК', 'steel', 16_000_000, 't', '12M2025',
                   'incl. NLMK USA/Europe'),
]

POLYUS_PRODUCTION = [
    ProductionData('Полюс', 'gold', 2_529_000, 'oz', '12M2025',
                   'факт из IR (-15.8% г/г)'),
]
...
PROFILES: Dict[str, CompanyProfile] = {
    'Норникель': CompanyProfile(
        ...
        revenue_share={'copper': 0.30, 'nickel': 0.25,
                       'palladium': 0.30, 'platinum': 0.08,
                       'byproducts': 0.07},  # Au+Co+Rh+Ir+Te+Se ~7% выручки
...
ACTUAL_REVENUE_12M_2025 = {
    'Норникель': {'usd_bn': 13.763, 'rub_bn': None,
                  'source': 'МСФО 12М 2025 (МЕТ-001)'},
    'Северсталь': {'usd_bn': None, 'rub_bn': 712.9,
                   'source': 'МСФО 12М 2025 (МЕТ-004)'},
    'ММК': {'usd_bn': None, 'rub_bn': 609.87,
            'source': 'МСФО 12М 2025 (МЕТ-005)'},
    'НЛМК': {'usd_bn': None, 'rub_bn': 831.35,
             'source': 'МСФО 12М 2025 (МЕТ-006)'},
    'Полюс': {'usd_bn': 8.723, 'rub_bn': None,
              'source': 'МСФО 12М 2025 (МЕТ-010)'},
}
...
def predict_hybrid(
    ...
    for prod in production:
        ...
        domestic_price_rub = (prices['steel_fob_chm'].avg_price_usd *
                              fx.avg_usd_rub *
    ...
    total_rub = (total_usd * fx.avg_usd_rub + total_rub_direct) * \
                (1 + profile.other_income_pct)
...
def predict_revenue(company: str, fx: FXRate = FX_12M_2025) -> RevenuePredict:
    ...
    if profile.revenue_model == 'global_commodity':
        return predict_global_commodity(company, production,
                                          PRICES_12M_2025, profile, fx)
    elif profile.revenue_model == 'hybrid':
        return predict_hybrid(company, production,
                                PRICES_12M_2025, profile, fx)
    else:
...
def main():
    ...
    parser.add_argument('--company',
                        ...
                        default='all', help='Эмитент для бэк-теста')
    args = parser.parse_args()

    targets = (['Норникель', 'Северсталь', 'ММК', 'НЛМК', 'Полюс']
    ...
    for company in targets:
        ...
        if result.actual_usd_bn is not None:
            print(f'  Факт МСФО: ${result.actual_usd_bn:.3f} млрд '
    ...
    print(f'  ✅ MAE ≤ 10% (success): {len(success)} — '
          f'{[r.company for r in success]}')
    print(f'  ⚠️ MAE 10-20% (acceptable): {len(accept)} — '
          f'{[r.company for r in accept]}')
    print(f'  ❌ MAE > 20% (needs work): {len(fail)} — '
          f'{[r.company for r in fail]}')
    print()
