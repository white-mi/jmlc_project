...
LAYER_MARK = {'full': '✅', 'validated': '✅ структ', 'structural_deferred': '◐ learned',
              'illustrative': '○ иллюстр', 'illustrative_pending': '○ pending'}
TIER_RU = {'validated': 'валидирован', 'validated_structural_deferred': 'валидирован (структ. отложена)',
           'illustrative': 'иллюстративный', 'illustrative_pending': 'иллюстративный (pending)'}
...
def _live_metrics(rel_path: str, industry: str):
    ...
    return {'panel_n': _panel_n(industry), 'n_common': d.get('n_common'), 'base': d.get('base'),
            'winner': win[0], 'winner_mape': win[1]}
...
def render_markdown(manifest=None) -> str:
    ...
    for r in rs:
        ...
        L.append(f'| **{RU.get(r["industry"], r["industry"])}** | ' + ' | '.join(cells)
                 + f' | {TIER_RU.get(r["tier"], r["tier"])} |')
    L += ['', '✅ полный · ◐ частичный (структурная отложена → learned) · ○ иллюстративный (бектест-only)',
    ...
    for r in rs:
        ...
        ncell = f'{pn} ({nc})' if pn is not None and nc is not None else (nc if nc is not None else '—')
        rep = f'[{Path(r["ds_report"]).name}]({Path(r["ds_report"]).name})' if r['ds_report'] else '—'
        L.append(f'| {RU.get(r["industry"], r["industry"])} | {ncell} | {m.get("winner", "—")} | {mape} | {rep} |')
