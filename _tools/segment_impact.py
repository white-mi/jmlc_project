...
def predict_segment_impact(
    ...
    if region and region not in REGION_PROFILES:
        raise ValueError(f"Неизвестный region: {region}. Доступны: {list(REGION_PROFILES)}")
...
def coverage_check(table_path: Path = DATA_PATH) -> dict:
    ...
    subs = {k: v for k, v in table["shock_subcategories"].items() if not k.startswith("_")}
        ...
        for ch in expected_channels:
            if ch not in sub_def.get("channels", {}):
                ...
                if "direction" not in cd:
                    ...
                    issues.append(
                        f'shock {sub_id}.{ch}: direction must be ±1, got {cd["direction"]}'
                    )
...
def main():
    ...
    parser.add_argument(
        "--kc-regime", default="acute_stress", choices=["normal", "moderate_stress", "acute_stress"]
    )
    parser.add_argument(
        "--direction",
        type=int,
    ...
    parser.add_argument(
        "--breakdown", action="store_true", help="Показать вклад каждого канала в ΔPD сегмента"
    )
    parser.add_argument("--check", action="store_true", help="Проверить покрытие таблицы")
    ...
    for sgmt, imp in impacts.items():
        print(
            f"  {sgmt:<15} | {imp.delta_pd:+.3f} | {imp.delta_demand:+8.2f}% | "
