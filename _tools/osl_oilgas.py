...
PRICES_12M_2025: Dict[str, CommodityPrice] = {
    ...
    "urals": CommodityPrice("urals", "barrel", 56.0, "12M2025", "Минфин: средняя реализация 2025"),
    "gas_export_china": CommodityPrice(
        "gas_export_china", "kcm", 280, "12M2025", "Сила Сибири price"
    ),
    "gas_export_other": CommodityPrice(
        "gas_export_other", "kcm", 320, "12M2025", "Турция + СНГ + остатки EU"
    ),
    "gas_domestic_rub": CommodityPrice(
        "gas_domestic_rub", "kcm", 7_500, "12M2025", "Регулируемая цена для пром-ти + население"
    ),
    "lng_jkm": CommodityPrice(
        "lng_jkm", "mmbtu", 10.0, "12M2025", "Долгосрочные контракты Ямал-СПГ ниже JKM-spot ($13.5)"
    ),
    "gas_condensate": CommodityPrice("gas_condensate", "barrel", 50, "12M2025", "к Urals -10%"),
    "refined_product_premium": CommodityPrice(
        "refined_product_premium", "usd_t", 250, "12M2025", "Маржа downstream в $/т сверх Urals"
    ),
}
...
ROSNEFT_PRODUCTION = [
    ...
    ProductionData("Роснефть", "oil", 192_000_000, "t", "12M2025", "жидк. УВ ~192 млн т (оценка)"),
    ProductionData("Роснефть", "gas", 65_000, "mmcm", "12M2025", "попутный + сухой газ"),
    ProductionData(
        "Роснефть", "refined_oil", 88_000_000, "t", "12M2025", "переработка на НПЗ группы"
    ),
]
...
GAZPROM_PRODUCTION = [
    ProductionData(
        "Газпром", "gas", 340_000, "mmcm", "12M2025", "340 млрд м³ — потеря ЕС-рынка частично"
    ),
    ProductionData(
        "Газпром",
        "oil",
...
NOVATEK_PRODUCTION = [
    ...
    ProductionData(
        "Новатэк", "lng", 13_200_000, "t", "12M2025", "Ямал-СПГ + Арктик-СПГ-2; дисконт от санкций"
    ),
...
PROFILES: Dict[str, CompanyProfile] = {
    ...
    "Газпром": CompanyProfile(
        ...
        refining_share=0.25,
        petchem_share=0.03,
        other_share=0.30,  # +Газпром нефть
    ...
    "Новатэк": CompanyProfile(
        ...
        other_share=0.10,
        ndpi_share=0.20,
        export_share=0.40,  # Большая часть газа = внутр. рынок; LNG отдельно
        discount_to_brent=0,
...
ACTUAL_REVENUE_12M_2025 = {
    ...
    "ЛУКОЙЛ": {
        "rub_bn": 3_768,
        "source": "МСФО 12М 2025 — выручка 3.768 трлн ₽ (-15%); чистый убыток -1.06 трлн от списания зарубежных активов",
    },
    "Газпром": {
        "rub_bn": 7_000,
        "source": "оценка из 1H 4.99 трлн + 9M прибыль 1.117 трлн; полный отчёт ещё финализуется",
    },
...
ACTUAL_REVENUE_9M_2025 = {
    "Роснефть": {"rub_bn": 6_170, "source": "МСФО 9М 2025 — 6.17 трлн ₽", "period_share": 0.749},
    "ЛУКОЙЛ": {"rub_bn": 2_830, "source": "МСФО 9М 2025 — 2.83 трлн ₽", "period_share": 0.751},
    "Газпром": {
        "rub_bn": 5_200,
        "source": "оценка 9М 2025: 1H 4.99 + ~0.4 за 3Q",
        "period_share": 0.743,
    },
    "Новатэк": {"rub_bn": 1_080, "source": "МСФО 9М 2025 — ~1.08 трлн ₽", "period_share": 0.747},
}
...
def predict_gas_vink(
    ...
    # НДПИ от gross
    gross_taxable_rub = (
        rev_export_usd * fx.avg_usd_rub + rev_domestic_rub + rev_oil_usd * fx.avg_usd_rub
    )
...
def main():
    ...
    targets = (
        ["Роснефть", "ЛУКОЙЛ", "Газпром", "Новатэк"] if args.company == "all" else [args.company]
    )
