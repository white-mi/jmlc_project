...
def _is_uncertainty_text(text: str) -> bool:
    ...
    return (
        _has_term_from(words, ECONOMIC_TERMS)
        and _has_term_from(words, UNCERTAINTY_TERMS)
        and _has_term_from(words, POLICY_CRISIS_TERMS)
    )
...
def compute_epu(
    ...
    period = (
        ...
        if start_date
        else f"all ... {end_date.strftime('%Y-%m-%d')}"
    )
...
def main():
    ...
    parser.add_argument(
        "--end-date", default=None, help="Якорная дата окна YYYY-MM-DD (по умолчанию now)"
    )
    parser.add_argument(
        "--source",
        choices=["corpus", "fred"],
    ...
    print("=" * 70)
    print(
        f"  EPU general:  {result.epu_value:.1f} / 100  "
        f"({result.n_uncertainty_texts}/{result.n_total_texts} текстов)"
    )
    print(
        f"  EPU negative: {result.epu_negative_pct:.1f}%  "
