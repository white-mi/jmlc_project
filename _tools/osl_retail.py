...
WILDBERRIES_SIGNALS = [
    OperationalSignal('Wildberries', 'gmv', 6_100, 'млрд_руб', '12M2025',
                       'WB IR; +49% г/г'),
]

# Ozon: GMV ~2.3 трлн ₽; выручка ~998 млрд ₽
OZON_SIGNALS = [
    OperationalSignal('Ozon', 'gmv', 2_300, 'млрд_руб', '12M2025',
                       'Ozon IR; +60% г/г'),
]
...
GOLDEN_APPLE_SIGNALS = [
    OperationalSignal('Золотое яблоко', 'revenue', 205.1, 'млрд_руб', '12M2025',
                       '+32% г/г'),
]

# Лемана Про (DIY) — выручка ~451 млрд ₽
LEROY_SIGNALS = [
    OperationalSignal('Лемана Про', 'revenue', 451, 'млрд_руб', '12M2025',
                       '−6% г/г второй год подряд'),
]

# М.Видео-Эльдорадо (БТиЭ) — выручка ~451 млрд ₽
MVIDEO_SIGNALS = [
    OperationalSignal('М.Видео', 'revenue', 451, 'млрд_руб', '12M2025',
                       'убыток 20.1 млрд'),
]
...
PROFILES = {
    'Wildberries': CompanyProfile(
        name='Wildberries', business_model='marketplace',
        take_rate=0.155,  # ~15.5% комиссия + сервисы
        notes='Лидер RU-маркетплейсов'
    ),
    'Ozon': CompanyProfile(
        name='Ozon', business_model='marketplace',
        take_rate=0.235,  # ~23.5% комбинированная
        notes='Активная монетизация'
    ),
    'Melon Fashion': CompanyProfile(
        name='Melon Fashion', business_model='omnichannel',
        notes='Befree, Zarina, Sela; +22% выручка'
    ),
    'Золотое яблоко': CompanyProfile(
        name='Золотое яблоко', business_model='omnichannel',
        notes='Beauty, +32% выручка, ЧП ×2'
    ),
    'Лемана Про': CompanyProfile(
        name='Лемана Про', business_model='diy',
        notes='Бывший Леруа Мерлен; −6% второй год'
    ),
    'М.Видео': CompanyProfile(
        name='М.Видео', business_model='omnichannel',
        notes='БТиЭ + е-com; убыток'
    ),
}
...
def predict_marketplace(company: str, signals: List[OperationalSignal],
    ...
    return RevenuePredict(
        ...
        predicted_rub_bn=total, breakdown_rub_bn=breakdown
    )


def predict_omnichannel(company: str, signals: List[OperationalSignal],
                          profile: CompanyProfile) -> RevenuePredict:
    """
    ...
    return RevenuePredict(
        company=company, period='12M2025',
        predicted_rub_bn=rev, breakdown_rub_bn=breakdown
    )
...
def main():
    ...
    parser.add_argument('--company',
                        choices=['Wildberries', 'Ozon', 'Melon Fashion',
                                 'Золотое яблоко', 'Лемана Про', 'М.Видео', 'all'],
                        default='all')
