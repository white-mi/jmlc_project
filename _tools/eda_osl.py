...
INDUSTRY_FX_BASKET = {
    'metallurgy': ('Норникель', [('vol_copper_t', 'copper'),
                                 ('vol_nickel_t', 'nickel'),
                                 ('vol_pt_oz', 'platinum')]),  # без Pd/Au (нет цены/объёма)
}
...
def fig_revenue_trends(df, ctx) -> str:
    ...
    fig.savefig(ctx.out / '01_revenue_trends.png', dpi=110); plt.close(fig)
    return ('01 Динамика выручки: индексация снимает разницу масштаба/валюты. Видно расхождение '
...
def fig_elasticity(df, ctx) -> str:
    ...
    if not issuers:
        ...
        fig.savefig(ctx.out / '02_elasticity.png', dpi=110); plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(ctx.out / '02_elasticity.png', dpi=110); plt.close(fig)
    tail = ('. Структурная модель неявно предполагает наклон=1 (выручка=Q×P); отклонения '
            'эмпирического наклона (хедж, лаги, доля рынка, бленд продуктов) — то, что '
            'learned-модель может поймать.')
    if ctx.industry == 'metallurgy':
        tail += (' Для сталеваров iron_ore — ВХОДНАЯ цена (прокси), не выходная: знак связи '
...
def fig_fx_passthrough(df, ctx) -> str:
    ...
    if not cfg:
        fig, ax = plt.subplots(figsize=(7, 4)); ax.axis('off')
        ax.text(0.5, 0.5, 'FX-реконструкция неприменима\n(выручка отрасли в ₽)',
                ha='center', va='center', fontsize=11)
        fig.savefig(ctx.out / '03_fx_passthrough.png', dpi=110); plt.close(fig)
        return ('03 FX pass-through: ПРОПУЩЕНО — у отрасли нет USD-ценовой сырьёвой корзины для '
...
def fig_volume_revenue(df, ctx) -> str:
    ...
    fig.tight_layout()
    fig.savefig(ctx.out / '04_volume_revenue.png', dpi=110); plt.close(fig)
    return ('04 Объём↔выручка (нормированы). Где связь слабее «чистого» Q×P — значит цена и микс '
...
def fig_missingness(df, ctx) -> str:
    ...
    fig.tight_layout()
    fig.savefig(ctx.out / '05_missingness.png', dpi=110); plt.close(fig)
    ...
    if pct < 3:
        return (f'05 Пропуски: всего {pct:.0f}% ячеек драйверов пусты — панель ПЛОТНАЯ (объёмы '
    ...
    return (f'05 Пропуски: {pct:.0f}% ячеек драйверов пусты (объёмы — {vol_miss:.0f}%). Структурно '
...
def fig_target_distribution(df, ctx) -> str:
    ...
    axes[0].hist(t, bins=10); axes[0].set_title('Таргет (млрд, родная валюта)')
    axes[1].hist(np.log(t), bins=10); axes[1].set_title('log(таргет)')
    for a in axes:
        a.set_ylabel('частота')
    fig.tight_layout()
    fig.savefig(ctx.out / '06_target_distribution.png', dpi=110); plt.close(fig)
    mix = ' (а в металлургии ещё и смешение USD/RUB)' if ctx.industry == 'metallurgy' else ''
    return (f'06 Масштаб таргета: от {t.min():.1f} до {t.max():.0f} — разброс уровней эмитентов{mix}. '
...
def fig_price_correlation(df, ctx) -> str:
    ...
    fig.tight_layout()
    fig.savefig(ctx.out / '07_price_correlation.png', dpi=110); plt.close(fig)
    ...
    if n_per > len(sub) >= 2:
        ...
        vif_part = (f'VIF (сокр. набор {"/".join(c.replace("p_", "") for c in sub)}, n>p): '
    ...
    if strong:
        impl = ('Импликация: высокие VIF/|corr| ⇒ OLS неустойчив → регуляризация (ElasticNet/Ridge) '
                'оправдана; на малом N плюс отбор/сжатие цен.')
    else:
        impl = ('Импликация: коллинеарность умеренная ⇒ регуляризация менее критична, но на малом N '
                'всё равно желательна для устойчивости.')
    return (f'07 Мультиколлинеарность цен (n={n_per} периодов). {vif_part}'
...
def fig_structural_residuals(df, ctx) -> str:
    ...
    if errs:
        ...
        ax.axis('off')
        ax.text(0.5, 0.5, 'структурная модель отложена\n(NaN на всех строках панели)',
                ha='center', va='center', fontsize=11)
    fig.tight_layout()
    fig.savefig(ctx.out / '08_structural_residuals.png', dpi=110); plt.close(fig)
    if not errs:
        return ('08 Структурный бейзлайн ОТЛОЖЕН для отрасли (StructuralOSL даёт NaN на всех строках '
    ...
    thin_note = (f' Тонкие эмитенты (n≤2: {", ".join(thin)}) дают почти точный in-sample фит — '
                 'их низкая ошибка не информативна.') if thin else ''
    return (f'08 Бейзлайн StructuralOSL (IN-SAMPLE, per-period цены/объёмы + скаляр k): средн |ошибка| '
...
def main():
    ...
    ap.add_argument('--industry', default='metallurgy',
                    help='metallurgy / oilgas / chemistry / energy')
    args = ap.parse_args()
