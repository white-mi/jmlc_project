...
def main():
    ...
    out.append(
        "> L0 — реальный 5-агентный LLM-конвейер (подагенты Claude Code). "
    ...
    out.append(
        "| # | Новость | Подкат. | Сила | Dir | L2 источник→топ | Худший сегмент (ΔPD) | Лучший сегмент (ΔPD) | Бифурк. |"
    )
    out.append("|---|---|---|---|---|---|---|---|---|")
    ...
    out.append(
        f"- **Информационный шум (shortcut=true):** {n_short}/{n_total} "
        "(в production — однострочная пометка без L1-L3)"
    )
    out.append(
        f"- **Расхождений маршрутизации отрасли (LLM primary_industry ≠ L2 source pipeline):** "
        f"{n_diverge}/{n_total}"
    )
    if n_diverge:
        ...
        for r in rows:
            if r["is_diverge"]:
                out.append(
                    f'  - {r["id"]} [{r["subcat"]}]: LLM={r["llm_industry"]} → pipeline={r["l2_source"]}'
                )
        out.append(
    ...
    print(
        f"   Бифуркаций: {n_bif}/{n_total} · shortcut: {n_short}/{n_total} · "
        f"расхождений маршрутизации: {n_diverge}/{n_total}",
        file=sys.stderr,
    )
