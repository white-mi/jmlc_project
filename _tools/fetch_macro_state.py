...
def fetch_inflation_yoy(timeout: int = 10) -> Optional[float]:
    ...
    url = (
        "https://api.worldbank.org/v2/country/RU/indicator/"
...
def main():
    ...
    parser.add_argument(
        "--set", nargs="*", default=None, help="Ручной апдейт key=value (например key_rate=15)"
    )
    ...
    if not values:
        print(
            "  ⚠️ Нет новых значений (сеть недоступна и --set не задан). "
            "current_state без изменений.",
            file=sys.stderr,
        )
        return

    result = update_macro_state(values, period=args.period, note=args.note, dry_run=args.dry_run)
