...
- **253 pytest зелёных, 0 skipped** (+10 DS-нефтегаз; +9 DS-химия; +EDA-параметризация и DS-синтез).
...
```bash
cd _tools
python -m pytest tests/ -v              # 253 зелёных, 0 skipped
```
...
| **D (частично)** | Расширение тестов — 253 зелёных (0 skipped) |
