...
Координировать последовательный запуск 5 агентов на одной новости. Доступны два режима: **ручной** — через Claude Code, и **автоматизированный** — Python-скрипт `orchestrator.py`.

## Workflow (ручной режим)
...
## Workflow (автоматизированный — `orchestrator.py`)
...
3. Перед Agent 3 вызывает `find_analogs()` из `rag/` (top_k=5; порог берётся по эмбеддеру из `embeddings.DEFAULT_THRESHOLDS`, переопределяется env `RAG_MIN_SIMILARITY`) и подсовывает аналоги в промпт
...
- Параллельный запуск Agent 2 и Agent 3 (сейчас последовательный: Agent 3 получает контекст от Agent 2).
- Self-критика: Agent 4.5 ревьюит выводы Agent 4 перед сборкой отчёта.
