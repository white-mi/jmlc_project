...
def _stub_response() -> str:
    return ('{"dry_run": true, "WHAT": "stub", "main_category": "1.1", '
...
def call_llm_sdk(prompt: str, model: str = DEFAULT_MODEL,
    ...
    try:
        ...
        raise ImportError(
            "anthropic SDK не установлен. Run: pip install anthropic"
        )
...
def call_llm(prompt: str, model: str = DEFAULT_MODEL,
             mode: str = DEFAULT_LLM_MODE, max_tokens: int = 8000) -> str:
...
def fetch_analogs(query_text: str, top_k: int = 5,
                  subcategory_filter: Optional[str] = None) -> list[dict]:
...
def run_agent(agent_num: int, substitutions: dict, model: str,
              llm_mode: str = DEFAULT_LLM_MODE, parse_json: bool = True) -> Any:
    ...
    print(f"  -> Agent {agent_num} ({agent_files[agent_num]})...",
          file=sys.stderr)
...
def run_pipeline(news_text: str, source: str, date: str,
                 bifurcation: bool = False, model: str = DEFAULT_MODEL,
    ...
    # Agent 1
    state = run_agent(1, {
        'НОВОСТЬ': news_text, 'ИСТОЧНИК': source, 'ДАТА': date,
    }, model=model, llm_mode=llm_mode)

    if state.get('shortcut'):
        print("  -> shortcut=true: skip Agent 2-4", file=sys.stderr)
        markdown = run_agent(5, {'STATE_JSON': state, 'MODE': 'shortcut'},
                             model=model, llm_mode=llm_mode, parse_json=False)
    ...
    state = run_agent(3, {'STATE_JSON': state, 'RAG_ANALOGS': analogs},
                      model=model, llm_mode=llm_mode)
    ...
    state = run_agent(4, {'STATE_JSON': state, 'BIFURCATION': bifurcation,
                          'OSL_RESULTS': osl_results},
                      model=model, llm_mode=llm_mode)

    # Agent 5
    markdown = run_agent(5, {'STATE_JSON': state}, model=model,
                         llm_mode=llm_mode, parse_json=False)
...
def main():
    parser = argparse.ArgumentParser(
        description='Multi-Agent Pipeline для Макро-радара (v0.7)')
    parser.add_argument('--news-file', help='Файл с текстом новости')
    parser.add_argument('--source', required=True,
                        help='Источник новости (URL или издание)')
    parser.add_argument('--date',
                        default=datetime.now().strftime('%Y-%m-%d'),
                        help='Дата новости (YYYY-MM-DD)')
    ...
    parser.add_argument('--dry-run', action='store_true',
                        help='Алиас --llm-mode dry-run (legacy)')
    parser.add_argument('--no-save', action='store_true',
                        help='Не сохранять результат в _Анализы/')
    parser.add_argument('--no-reindex', action='store_true',
                        help='Не запускать index_news после сохранения')
    ...
    print(f'  Multi-Agent Pipeline · model={args.model} · llm_mode={llm_mode}',
          file=sys.stderr)
    print('=' * 70, file=sys.stderr)
    ...
    state, markdown = run_pipeline(
        news_text, args.source, args.date,
        bifurcation=args.bifurcation, model=args.model,
        llm_mode=llm_mode,
    )
    ...
    if not args.no_save:
        ...
        if not args.no_reindex:
            ok = reindex_rag(path)
            print(f'  RAG re-index: {"✅" if ok else "⚠️ skipped"}',
                  file=sys.stderr)
    else:
